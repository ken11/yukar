...
test.describe
  .serial("multi-trial fake smoke", () => {
    ...
    test("After creating a new trial, the URL navigates to the new thread id", async ({ page }) => {
      ...
      // New trial button — target the button inside the persistent thread list
      // (nav[aria-label="Threads"]) in the desktop sidebar.
    ...
    test("Both old and new trials appear in the thread list pane", async ({ page }) => {
      ...
      // The persistent thread list is always present in the desktop sidebar.
