...
the claim "reloading while awaiting_input replays the question bubble" is
proved (or falsified) by running code rather than by static reasoning.

Scenarios
---------
(A) RunStarted → UserInputRequestedEvent published → NEW subscribe() (reload
    equivalent) → backfill must contain UserInputRequestedEvent with question.
    If this test FAILS the backend replay is broken.
    If it PASSES the backend is correct and the bug, if any, is in the frontend.

(B) UserInputRequestedEvent + UserInputResolvedEvent both published → new
...
Coverage-gap note
-----------------
test_ask_user_gate.py::TestUserInputRequestedBusReplay only checks that the
...
def _uir(project_id: str, epic_id: str, question: str = "Plan OK?") -> Any:
    """Build a UserInputRequestedEvent."""
...
def _uiresolved(project_id: str, epic_id: str) -> Any:
    """Build a UserInputResolvedEvent."""
    from yukar.models.events import UserInputResolvedEvent

    return UserInputResolvedEvent(
        **_base(project_id=project_id, epic_id=epic_id),
        thread_id="manager",
    )
...
# ---------------------------------------------------------------------------
# Scenario (A): New subscribe() receives UserInputRequestedEvent via backfill
# ---------------------------------------------------------------------------
...
class TestScenarioA_ReloadBackfill:
    ...
    "ask_user question disappears when the page is reloaded".
    ...
    async def test_new_subscriber_receives_uir_via_backfill(self) -> None:
        ...
        Timeline:
          ...
          3. UserInputRequestedEvent published → enters _replay deque.
          4. No live subscribers at this point (simulating "between connections").
          5. NEW subscribe() opened (= browser reload / re-connect).
          6. Backfill flush must deliver UserInputRequestedEvent with question.
        """
        ...
        # Step 3: UserInputRequestedEvent — Manager asked the user a question.
        question = "Plan OK?"
        event_bus.publish(project_id, epic_id, _uir(project_id, epic_id, question=question))

        # Verify it is in _replay before subscribing.
        buf_before = list(event_bus._replay[(project_id, epic_id)])
        uir_in_buf = [e for e in buf_before if isinstance(e, UserInputRequestedEvent)]
        assert uir_in_buf, (
            "PRECONDITION FAILED: UserInputRequestedEvent not in _replay buffer. "
        ...
        uir_received = [e for e in received if isinstance(e, UserInputRequestedEvent)]
        assert len(uir_received) >= 1, (
            "BACKEND REPLAY BROKEN: UserInputRequestedEvent was NOT delivered to a "
            "new subscriber via the replay buffer. A browser reload while awaiting_input "
            "will lose the question bubble. "
            f"Received events: {[getattr(e, 'type', type(e).__name__) for e in received]}"
        )
        assert uir_received[0].question == question, (
            f"Question text mismatch: got {uir_received[0].question!r}, "
    ...
    async def test_backfill_delivers_uir_after_any_intermediate_events(self) -> None:
        ...
        event_bus.publish(project_id, epic_id, _run_started(project_id, epic_id))
        # 10 pause/resume cycles before the question.
        for _ in range(10):
            event_bus.publish(project_id, epic_id, _run_paused(project_id, epic_id))
            event_bus.publish(project_id, epic_id, _run_resumed(project_id, epic_id))
        # UIR is the 22nd event in the deque (1 RunStarted + 20 pause/resume + 1 UIR).
        event_bus.publish(
            project_id, epic_id, _uir(project_id, epic_id, question="Still OK after 10 cycles?")
        )
        ...
        assert uir_received[0].question == "Still OK after 10 cycles?"
...
class TestScenarioB_ResolvedAfterRequested:
    ...
    The frontend reducer should process:
      user_input_requested → set awaiting state
      user_input_resolved  → clear awaiting state
    ...
    async def test_request_then_resolved_both_replayed_in_order(self) -> None:
        ...
        event_bus.publish(project_id, epic_id, _run_started(project_id, epic_id))
        event_bus.publish(project_id, epic_id, _uir(project_id, epic_id, question="Proceed?"))
        ...
        assert "user_input_requested" in types, (
            f"user_input_requested not in replay. types: {types}"
        )
        assert "user_input_resolved" in types, (
            f"user_input_resolved not in replay. types: {types}"
        )
        # resolved must appear AFTER requested so reducer can transition correctly.
        uir_idx = types.index("user_input_requested")
        resolved_idx = types.index("user_input_resolved")
        assert resolved_idx > uir_idx, (
            f"user_input_resolved ({resolved_idx}) must come after "
            f"user_input_requested ({uir_idx}) in replay. types: {types}"
        )
    ...
    async def test_resolved_clears_awaiting_state_for_late_subscriber(self) -> None:
        ...
        question bubble when the user has already answered.
        """
        from yukar.events import bus as event_bus
        ...
        event_bus.publish(project_id, epic_id, _run_started(project_id, epic_id))
        event_bus.publish(
            project_id, epic_id, _uir(project_id, epic_id, question="Already answered")
        )
        event_bus.publish(project_id, epic_id, _uiresolved(project_id, epic_id))
        ...
        for ev in received:
            if isinstance(ev, UserInputRequestedEvent):
                awaiting = True
            elif isinstance(ev, UserInputResolvedEvent):
                awaiting = False

        assert not awaiting, (
            "Late subscriber reducer would show question bubble even though "
            "user already answered. Frontend would display stale awaiting UI. "
    ...
    async def test_only_request_no_resolve_leaves_awaiting(self) -> None:
        ...
        event_bus.publish(
            project_id,
            epic_id,
            _uir(project_id, epic_id, question="Still waiting for your answer"),
        )
        # No UserInputResolvedEvent — user hasn't answered yet.
        ...
        assert awaiting, (
            "Late subscriber must see awaiting_input state when only UIR (no resolved) "
            "was published. Backend replay is broken — question bubble cannot be shown. "
...
class TestScenarioC_EvictionRisk:
    ...
    def test_uir_evicted_when_total_exceeds_maxlen(self) -> None:
        ...
        event_bus.publish(
            project_id, epic_id, _uir(project_id, epic_id, question="Will I be evicted?")
        )
        ...
        uir_in_buf = [e for e in buf if isinstance(e, UserInputRequestedEvent)]
    ...
    def test_uir_survives_when_total_at_maxlen(self) -> None:
        ...
        event_bus.publish(
            project_id,
            epic_id,
            _uir(project_id, epic_id, question="I should survive"),
        )
    ...
    def test_uir_survives_when_just_below_eviction_boundary(self) -> None:
        ...
        event_bus.publish(project_id, epic_id, _run_started(project_id, epic_id))
        event_bus.publish(
            project_id, epic_id, _uir(project_id, epic_id, question="Boundary check")
        )
    ...
    async def test_new_subscriber_does_not_receive_evicted_uir(self) -> None:
        ...
        genuinely broken for late subscribers — the question bubble cannot be
        ...
        event_bus.publish(project_id, epic_id, _run_started(project_id, epic_id))
        event_bus.publish(project_id, epic_id, _uir(project_id, epic_id, question="Evicted Q"))
    ...
    async def test_uir_survives_subscribe_backfill_within_maxlen(self) -> None:
        ...
        event_bus.publish(project_id, epic_id, _run_started(project_id, epic_id))
        question = "End-to-end eviction + backfill"
        event_bus.publish(project_id, epic_id, _uir(project_id, epic_id, question=question))
...
class TestCoverageGapDocumented:
    ...
    test_ask_user_gate.py::TestUserInputRequestedBusReplay.test_uir_event_in_replay_buffer
    ...
    def test_gap_documented_prior_test_only_checks_replay_buffer(self) -> None:
        ...
        Specifically: test_uir_event_in_replay_buffer asserts
            event_bus._replay[key] contains UserInputRequestedEvent
