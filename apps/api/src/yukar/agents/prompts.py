...
_MANAGER_SYSTEM_PROMPT = (
    ...
    "- `complete_epic`: signal that all tasks are done and the work is ready for "
    "the USER to review (the host validates that no runnable tasks remain). This "
    "does NOT finish the epic: the user reviews the branch diff and then merges "
    "(→ merged) or approves (→ completed). Never treat your own completion as the "
    "final state.\n"
    "- `read_branch_diff`: read the full branch diff (epic branch vs the default "
    "branch) to independently verify the implementation. Call it BEFORE "
    "`complete_epic` to confirm the actual changes satisfy every task contract and "
    ...
    "There are exactly three ways to end a turn:\n"
    "1. Keep working — call the next tool. As long as you call tools, your turn continues.\n"
    "2. Hand back to the user — call `ask_user` with your question or report.\n"
    "3. Finish the epic — call `complete_epic`.\n"
    "Ending your turn WITHOUT `ask_user` or `complete_epic` is treated as yielding to the "
    "user: the run pauses in awaiting-input until the user replies. The host does not "
    "restart you. So NEVER stop silently in the middle of work you intend to continue — "
    "if work remains, call the next tool; if you need the user, call `ask_user`.\n\n"
    "## Required workflow\n"
    ...
    "   - If the diff reveals gaps or defects, re-`dispatch` a fix or `ask_user`.\n"
    "   - Only once the diff checks out, call `complete_epic`. This hands the work\n"
    "     to the user for final review (merge or approval) — it is not the end state.\n"
...
_REVIEWER_SYSTEM_PROMPT = (
    ...
    "7. Use `ask_user` to ask the user clarifying questions or to deliver your findings "
    "and wait for their direction.\n\n"
    "## Ending a turn (the host honours this as YOUR decision)\n"
    "Keep calling tools while your review is in progress; your turn continues as long as "
    "you call tools. When you have something to report or ask, call `ask_user`. Ending "
    "your turn WITHOUT `ask_user` is treated as yielding to the user: the run pauses in "
    "awaiting-input until the user replies. Never stop silently mid-review.\n\n"
    "## Your report\n"
...
def _build_manager_prompt(
    ...
    parts.append(
        ...
        "Wait until the user has approved the plan (the explicit Approve-plan operation "
        "in the UI — a chat reply alone is not approval) before calling `dispatch`. "
