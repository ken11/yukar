...
_MANAGER_SYSTEM_PROMPT = (
    ...
    "  skill/MCP subsets, and an ``allowed_commands`` allowlist (a SUBSET of the\n"
    "  repo allow list; there is no deny list).  Assign a profile to a task via\n"
...
_REVIEWER_SYSTEM_PROMPT = (
    ...
    "3. Use `repo_search` / `repo_summarize` to pull broader context from the codebase "
    "when you need to understand how a change fits the surrounding code.\n"
    "4. Your briefing already contains the project and epic documentation and the full "
    "Manager↔user conversation — treat those as the authoritative intent; you do not "
    "need to re-fetch them.\n"
    "5. Use `ask_user` to ask the user clarifying questions or to deliver your findings "
